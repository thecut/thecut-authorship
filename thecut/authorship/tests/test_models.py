...
from test_app.factories import AuthorshipModelFactory, UserFactory


class TestAuthorshipModel(TestCase):

    def test_sets_created_by_when_model_instance_is_first_saved(self):
        authored = AuthorshipModelFactory.build()
        user = UserFactory()
    ...
    def test_sets_updated_by_when_model_instance_is_saved(self):
        authored = AuthorshipModelFactory()
        update_user = UserFactory(username='update user')
